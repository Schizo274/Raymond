...
def Sort(FileName):
    index = [] # list for values of gpa
    with open(FileName, "r") as file:
        lines = file.readlines()
        for l in lines[1:]:
            split = l.split()
            gpa = (split[-1]) # extracting the gpa from the file
            index += [gpa]

    hasSwap = True
    tempIndexList = [] # a temporary list of indexes to swap the entries based on gpa
    ...
    while hasSwap:
        ...
        for i in range(len(index)):
            if i == len(index)-1:
                continue
            if index[i] < index[i+1]: # if value of first gpa is lesser than the second
                hasSwap = True
                tempIndexList[i], tempIndexList[i+1] = tempIndexList[i+1], tempIndexList[i]  # swap the value of items in temporary index
                index[i], index[i+1] = index[i+1], index[i] # swap the values of gpa in list
    for i in tempIndexList:
        print(lines[i + 1]) # print out the values
